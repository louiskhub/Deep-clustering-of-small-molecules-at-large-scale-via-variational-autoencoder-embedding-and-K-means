...
ATOM_FEATURES = {
    ...
    'degree': [0, 1, 2, 3, 4, 5, 6], # number of bonds the atom is involved in, size = 7
    'formal_charge': [-2, -1, 0, 1, 2, 3], # integer electronic charge assigned to atom, size = 6
...
class LocalFeatures:
    def __init__(self, mol, onehot = False, pca = False, ids = None, count=0):
        print(count)
        ...
        if pca:
            ...
            # except ValueError: # very rarely f_bonds has no dimension
            #     print(fb) 
...
def mol2local(mols, onehot = False, pca = False, ids = None):
    if ids is not None:
        return BatchLocalFeatures([LocalFeatures(mol, onehot, pca, iid, count) for count,(mol,iid) in enumerate(zip(mols,ids))])
    else:
...
if __name__ == '__main__':
    ...
    active_compound_df = pd.read_csv(
    ...
    # dirty_data = pd.read_csv("data/compound-annotation.csv")
    # cleaned_data = dirty_data.dropna(subset=["SMILES"], how="any")
    #data = data.iloc[:30,:]
    
    data_smiles = active_compound_df['decoded_infile_property'].values.tolist()
    data_id = active_compound_df['id']
    ...
    f_atoms_pca.columns = f_atoms_pca.columns.astype(str)
    f_bonds_pca.columns = f_bonds_pca.columns.astype(str)
    
    f_atoms_pca.to_feather('data/atom_features_PCA.feather', index=False)
    f_bonds_pca.to_feather('data/bond_features_PCA.feather', index=False)
